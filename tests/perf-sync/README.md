...
# 100 users total, but only 10 syncing at any one time.
node tests/perf-sync/cli.js baseline --users=100 --max-concurrent-users=10 \
  --protocol=pg-sync --run-id=load
```

`--max-concurrent-users` caps how many virtual users sync in parallel
while the scenario still runs all `--users` to completion. Leave it unset
to keep the previous behavior (all users sync concurrently). Only the
`baseline` and `initial-vs-ongoing` scenarios honor it; `compare-protocols`
is single-user by construction.
