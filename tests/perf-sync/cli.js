...
const printHelp = () => {
  process.stdout.write([
    ...
    '',
    'Options:',
    '  --users=<n>             total virtual users to run (default 1)',
    '  --max-concurrent-users=<n>  cap on users syncing at once (default --users)',
...
const buildContext = (parsed) => {
  ...
  if (!Number.isFinite(userCount) || userCount < 1) {
    throw new Error(`--users must be a positive integer (got ${parsed.flags.users})`);
  }
  const maxConcurrentRaw = parsed.flags['max-concurrent-users'];
  const maxConcurrentUsers = maxConcurrentRaw === undefined || maxConcurrentRaw === true
    ? userCount
    : parseInt(maxConcurrentRaw, 10);
  if (!Number.isFinite(maxConcurrentUsers) || maxConcurrentUsers < 1) {
    throw new Error(`--max-concurrent-users must be a positive integer (got ${maxConcurrentRaw})`);
  }
