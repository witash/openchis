...
const matchInsertDoc = () => containsSubstr('INSERT INTO medic_documents');
const matchUpdateSubject = () => containsSubstr('UPDATE medic_documents');
...
describe('postgres-sync importer', () => {
  ...
  describe('processBatch — doc update (new _rev)', () => {
    it('inserts a second row when the same _id arrives with a new _rev', async () => {
      ...
      expect(inserts.secondCall.args[1][3]).to.equal(JSON.stringify(v2.doc));
    });
  });
  ...
  describe('processBatch — soft delete', () => {
    it('writes a tombstone with deleted=true and removes any contact row', async () => {
      ...
      expect(insertCall.args[1][2]).to.equal('7');
      expect(JSON.parse(insertCall.args[1][3])).to.deep.equal({
        _id: 'doc-1',
        _rev: '3-tomb',
        _deleted: true,
      });
      expect(insertCall.args[1][4]).to.equal(null); // subject
      expect(insertCall.args[1][5]).to.equal(null); // type
      expect(insertCall.args[1][6]).to.equal(true); // deleted
